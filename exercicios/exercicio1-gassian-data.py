...
class dataset(object):
    ...
    def genContorno(self,correlacao,M1,lowLim,highLim):
        ...
        return M1 
    '''#########################################'''
    def maxProb(self,x,y,datasets):
        points, surface = [], []
        aux = datasets[0][0][0]
        print('whatfuck.',aux)
        i=0 
        while i < 4:
            surface = datasets.pop()
            aux = surface.pdfSurp[0][0][0]
            print(i,aux,'------')
            points.append(aux)
            i += 1
    ...
    '''
        maxProb,maxClass,classCount = 0,0,0
        for dataset in datasets:
            maxProb = max(maxProb, dataset.pdfSurp[0][x][y])
            if dataset.pdfSurp[0][x][y] > maxProb:
                maxProb = dataset.pdfSurp[0][x][y]
                maxClass = classCount
            print('maxProb',maxProb,'maxClass',maxClass,'classCount',classCount)
            if classCount == 4:
                return maxClass
            classCount += 1
    '''
    ...
    def genSeparacao(self,datasets=None):
        ...
        for i in seqi:
            ...
            for j in seqj:
                cj += 1
                aux  = self.maxProb(ci,cj,datasets)
                print(ci,cj,'class maxProb',aux)
                Mseparacao[ci-1][cj-1] = aux
    ...
    def plotContorno(self,datasets=None,lowLimit=None,highLimit=None):
        ...
        # contour
        fig, ax = plt.subplots()
        CS = ax.contourf(X,Y,separationSurface[0],5, cmap='tab20')
